...
const RULE_FIELDS=new Set(["id","category","type","title","description","strength","classificationRaw","status","verificationStatus","reviewStatus","review","affectedEntityIds","parameters","exceptions"]);
...
export function validateCopilotProposal(input:unknown,ctx:CopilotProposalContext):{ok:true;proposal:CopilotProposal}|{ok:false;message:string}{
  ...
  if(proposal.kind==="RULE_PATCH"){
    ...
    for(const key of Object.keys(changes))if(!RULE_FIELDS.has(key))return{ok:false,message:`AI rule proposal contains unsupported field ${key}.`};
    ...
    patch.proposedBy="AI";
    patch.reason=String(patch.reason||"AI-proposed Rulebook change");
  ...
  if(patch.operation!=="MOVE")return{ok:false,message:"V2.1 AI schedule proposals may only move an existing assignment."};
