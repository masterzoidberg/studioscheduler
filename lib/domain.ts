...
export interface ValidationResult { valid: boolean; fullyValidated: boolean; hardViolations: number; warnings: number; violations: ValidationViolation[]; coverage: ValidatorCoverage; }
export interface ScheduleVersion { id: string; version: number; rulebookVersion: number; createdAt: string; actor: string; reason: string; assignments: Assignment[]; isCurrent?: boolean; validationResult?: ValidationResult | null; }

export interface RulePatch {
  id: string; ruleId?: string; operation: "CREATE" | "UPDATE" | "RETIRE" | "DISABLE" | "ENABLE"; changes: Partial<StudioRule>;
  reason: string; proposedBy: "USER" | "AI"; baseRulebookVersion?: number; baseScheduleVersion?: number;
}
...
export interface Scenario { id: string; name: string; baseRulebookVersion: number; baseScheduleVersion: number; rulePatches: RulePatch[]; schedulePatches: SchedulePatch[]; createdAt: string; }
...
export interface StudioState { studioId: string; studioName: string; teachers: Teacher[]; rooms: Room[]; students: Student[]; cohorts: Cohort[]; classes: ClassDefinition[]; sessions: ClassSession[]; rules: StudioRule[]; rulebookVersions: RulebookVersion[]; ruleHistory: RuleHistoryEntry[]; scheduleVersions: ScheduleVersion[]; scenarios: Scenario[]; auditEvents: AuditEvent[]; }
