...
The manager product is incomplete: readiness and certification are implemented only for the current bounded setup slices, broader generation/review UX remains unfinished, onboarding is absent, normal UI exposes internal architecture, and schedule print/export are still missing. Governed session lock/unlock controls are now present with context invalidation and effective session-or-placement protection. Copilot still carries legacy authority assumptions. Tenant UUIDs, names, 178-rule accounting and calendar defaults prevent independent onboarding. Passing tests do not prove private data completeness, deployed migration state or independent manager use.
