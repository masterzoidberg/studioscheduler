...
Canonical baseline: `91204390d8c025789e6af871a5939eb22decd517`, 2026-09-07. Scope is a trustworthy weekly scheduling application that managers configure and operate independently. R0 plan rework adoption was completed on 2026-09-09 from audited implementation head `7f5c1287ec838cb6e8cc2e9efc395b13c6e426da`; POL-02 remains pending and POL-04 is the selected corrective run. [TASKS](TASKS.md) owns task status/dependencies; [NEXT](NEXT.md) selects one run. This plan supersedes prior release/generic/cutover roadmaps.

## What exists and what does not

Current code implements versioned policy/planning/schedules, deterministic Constraint IR, CP-SAT feasibility, coherent candidate context, per-session solver locks, authoritative manual commands/recovery and restricted legacy writes. T01–T13 are accepted historical work. Typed POL-02 application/runtime/compiler/solver/parity evidence is present, but R0 demonstrated that the effective SQL safeguard still covers legacy semantics only; POL-04 is the bounded forward-migration and no-write corrective. New P1 correctness finding: V49 adoption's nullable membership check fails to reject a missing row; SAFE-01 fixes this before new features.
...
The adopted execution route starts with R1/POL-04, which closes the demonstrated POL-02 SQL safeguard gap. R2 then unblocks the independent SET-03/04/05 setup slices; later runs follow the dependency graph in [TASKS](TASKS.md). This route is planning guidance only: no deployment, merge, external participant or private-data action is implied.
...
| Policy/rules/HARD validation | Typed TS/Python IR verified; POL-02 SQL safeguard parity pending; arbitrary prose unsupported | Typed supported bundles; fail closed | Tenant record accounting | Generic supported authoring | Stable documented vocabulary |
