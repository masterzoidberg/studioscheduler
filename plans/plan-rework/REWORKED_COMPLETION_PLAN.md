...
`TASKS.md` is the canonical status authority. At R0 start, audited HEAD `7f5c128` still said POL-02 was READY. The Git history after accepted SET-02 contains 27 POL-02 implementation/test commits and about 3,000 changed lines. R0 reconciled that implementation evidence against the prompt: typed application/runtime/compiler/solver/parity criteria passed, but the effective SQL safeguard criterion was a demonstrated gap. R1/POL-04 closed that gap at `15f5c79` with the V54 forward migration and executed disposable no-write coverage; POL-02 is now **DONE**. SET-03 then completed at `d46e68c`; `TASKS.md` now selects R2/SET-04 as the sole run.
...
3. **R2 — Core setup inputs (IN PROGRESS SEQUENCE).** SET-03 is complete with its bounded local evidence; the canonical ledger now selects SET-04 as the sole next task. SET-05 remains queued; later setup slices may be scheduled only by the canonical ledger after their dependencies and ownership are confirmed. Focused checks precede lane DB/parity/e2e integration.
...
**R2 — SET-04 teacher availability and qualifications.** Start from implementation head `d46e68c43ae920e99be40c7bd61e8a7d6c563728` on `feat/pre-cami-hardening`. Complete only SET-04: provide teacher availability windows/unavailable days and qualified class/subject domains through typed Rulebook policy, preserve explicit unknown versus unrestricted review state, and reject inferred qualification from notes. Preserve canonical version/context/tenant authority, add focused regressions, run the required lint/typecheck/unit/build/disposable DB/parity/authenticated E2E checks plus pinned Python checks for solver/IR changes, and stop on the runbook conditions. `TASKS.md` is the sole status/dependency ledger and `NEXT.md` is the sole selected-run view.
