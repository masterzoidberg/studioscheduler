# OPT-01 — Score schedule quality deterministically

Execution class: **STANDARD IMPLEMENTATION**. Milestone: **C**. Dependencies: **SET-07, VERIFY-01**.
Status is owned by [TASKS](../TASKS.md); do not infer readiness from this prompt existing.

## Outcome and current state

Add pure score breakdown for preferred/avoided days, teacher gap/compactness and student attendance days, then preferred room/teacher using explicit typed preference records. Lexicographic strength tiers preserve reviewed priority order; units are minutes/days/counts, no magic global weights. Missing preference contributes nothing and warns only.

Current evidence at audit HEAD `9120439`: Objective priority spine is metadata; no accepted scoring/optimization completion. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
No optimizer or promise of optimality.

## Tests and acceptance criteria

- Hand-calculated fixture totals and order stability, no HARD violation scoring as acceptable, explanatory labels reconcile totals
- No unreviewed preference invented from old prose.
