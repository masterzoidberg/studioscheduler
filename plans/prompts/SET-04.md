# SET-04 — Manage teacher availability and qualifications

Execution class: **STANDARD IMPLEMENTATION**. Milestone: **A**. Dependencies: **SET-03**.
Status is owned by [TASKS](../TASKS.md); do not infer readiness from this prompt existing.

## Outcome and current state

Provide allowed windows/unavailable days, earliest/latest times and eligible class/subject domains via typed Rulebook policy. Teacher identity remains planning data. Show explicit reviewed unrestricted availability vs unknown; qualifications require an explicit eligible domain, never infer from notes.

Current evidence at audit HEAD `9120439`: Teacher editor has name/color/notes; notes and subject labels are not executable availability or qualification authority. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
Do not store private reasons for parenting/other-job constraints when time windows suffice.

## Tests and acceptance criteria

- Availability edit invalidates only related slice; room edit preserves it
- Unqualified teacher rejected by TS/Python/server
- Conflicting windows shown with exact action; stale form refuses save and preserves draft text.
