...
Verify invite accept/expire/revoke, role changes and last-owner protection using selected studio. Define/export tenant data and owner-requested deletion workflow with documented retention/backup handling; minimize private student data and remove secrets from logs. Prepare support authorization boundaries.

Current evidence at audit HEAD `9120439`: OWNER/EDITOR/VIEWER and invitation concepts exist; external pilot account lifecycle/privacy not accepted. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
No automated billing, student accounts or unsolicited legal/compliance claims.

## Tests and acceptance criteria

- Revoked invite/session/member cannot mutate; last owner cannot orphan workspace; foreign tenant cannot export/delete; deletion rehearsal is disposable and auditable, backup limits accurately explained.
