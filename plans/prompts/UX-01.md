# UX-01 — Make schedule generation and failures understandable

Execution class: **STANDARD IMPLEMENTATION**. Milestone: **A**. Dependencies: **VERIFY-01, SET-07**.
Status is owned by [TASKS](../TASKS.md); do not infer readiness from this prompt existing.

## Outcome and current state

Create Build schedule action from Setup review; show bounded progress and review before adoption. Distinguish created, proven infeasible, unknown/timeout, incomplete, unsupported, unavailable and stale context. Cancellation ignores late result and leaves canonical schedule untouched. Human explanations link policy/lock actions; raw diagnostics in Advanced.

Current evidence at audit HEAD `9120439`: Gateway/feasibility exist, but successful and unsuccessful results are technical and fragmented. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
No minimum unsatisfiable-core algorithm, optimizer or background job service.

## Tests and acceptance criteria

- Synthetic timeout is never labeled impossible; infeasible never promises a solution; stale candidate cannot adopt
- Network failure preserves current schedule and offers retry
- Viewer cannot generate/adopt; submitting twice cannot silently duplicate adoption.
