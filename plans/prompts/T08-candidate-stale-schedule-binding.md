...
Task prerequisites: T07. If any prerequisite lacks verified DONE evidence, record the issue and do not bypass it. TASKS.md is the status authority; prompts are scope/acceptance references.

## Exact scope

Adoption currently uses the schedule freshly loaded at adoption rather than the schedule the manager reviewed.

Deliver only T08, using the current architecture. 
