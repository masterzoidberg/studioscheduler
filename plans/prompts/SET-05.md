...
Absorb reviewed structure/roster repairs into class details. Expose frequency and ordinal-specific durations, roster bulk selection, required/preferred teacher and room, explicit class scope. Eligibility is derived from policy, not a second writable eligibleTeacherIds authority. Register STRUCTURE/ROSTER review slices.

Current evidence at audit HEAD `9120439`: Classes support structure and roster repairs, but eligibility and review are not a unified manager flow. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
No automatic inference that empty roster is complete; no developer-created acceptance rosters.

## Tests and acceptance criteria

- Changing one session duration invalidates class structure review; unrelated roster review survives
- Confirm empty roster only when explicitly valid for that class scope; missing required participants block
- Archive referenced entity warns/rejects according to existing governed behavior.
