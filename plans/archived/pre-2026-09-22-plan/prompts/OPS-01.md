# OPS-01 — Verify deployment configuration and recovery
...
Write/run disposable restore and migration rehearsal, configuration validation and rollback-to-read-only procedure; verify separate staging app/solver auth, timeout limits and health checks when explicitly authorized. Prepare release checklist and exact expected versions; deployment remains separate owner authorization.

Current evidence at audit HEAD `9120439`: Build/SQL harness do not establish operational deployment, backups or Supabase auth fidelity. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
No deployment, production data access or paid provisioning without task-session authorization.

## Tests and acceptance criteria

- Reconstruct backup in isolated environment and reconcile version/hash/assignment counts; missing secrets fail visibly; migration failure does not lead to history rewrite
- Required real environment evidence must exist to mark DONE; recording an external gate leaves the task incomplete/BLOCKED while independent rehearsal can finish.
