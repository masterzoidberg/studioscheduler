# GEN-01 — Replace remaining name-bound targets

Execution class: **HIGH-REASONING IMPLEMENTATION**. Milestone: **B**. Dependencies: **UX-03, VERIFY-01**.
Status is owned by [TASKS](../TASKS.md); do not infer readiness from this prompt existing.

## Outcome and current state

Complete stable-ID typed parameters for remaining existing kinds, explicit progression/enrollment relations and unconditional qualification default-deny independent of CUR-007. Maintain historical artifact readers; canonical compiler emits typed forms only after parity.

Current evidence at audit HEAD `9120439`: POL-01 supports bounded typed forms; untouched historical/DWDE families still have name and curriculum coupling. For later tasks, predecessor behavior above is an expected contract, not a claim that it exists in this baseline.
...
No generic arbitrary expression language or unrelated duplicate-type cleanup.

## Tests and acceptance criteria

- Renamed/permuted/Unicode duplicate-name fixtures and transformed whole DWDE witness preserve legality
- No runtime source dispatch by DWDE person/rule ID
- All current kinds either fully supported or explicitly fail closed.
