...
Canonical baseline: `91204390d8c025789e6af871a5939eb22decd517`, 2026-09-07. Scope is a trustworthy weekly scheduling application that managers configure and operate independently. R0 plan rework adoption was completed on 2026-09-09 from audited implementation head `7f5c1287ec838cb6e8cc2e9efc395b13c6e426da`; R1/POL-04 was accepted at implementation head `15f5c79`, closing the demonstrated POL-02 SQL safeguard gap; SET-03 was accepted at implementation head `d46e68c`; SET-04 was accepted at implementation head `c165f4a`; SET-05 was accepted at implementation head `5f2aa78`; POL-03 was accepted at implementation head `e1a32d3`; SET-06 was accepted at implementation head `25a92a4`; SET-07 was accepted at implementation head `a763a75a11c4bfce96b6e8d52fb48dcac53b8b19`; UX-01 was accepted at implementation head `675878a`; UX-02 and LOCK-01 were accepted at implementation head `2fb44a0`; UX-03 was accepted at implementation head `63ea804`; GEN-02 through IMPORT-01 were accepted in the implementation worktree and frozen in implementation commit `a0a1900b6325f9792bd7868885f8b4e09daccfa6`; OPS-01 engineering rehearsal was completed at `607a8ce` with deployed evidence still external. OPT-01, OPT-02 and CAND-01 were accepted and frozen in the same implementation commit; R15/GEN-05 is selected and blocked on EXT-STUDIO2. [TASKS](TASKS.md) owns task status/dependencies; [NEXT](NEXT.md) selects one run. This plan supersedes prior release/generic/cutover roadmaps.

## What exists and what does not

Current code implements versioned policy/planning/schedules, deterministic Constraint IR, CP-SAT feasibility, durable tenant-scoped candidate review with coherent context comparison, per-session solver locks, authoritative manual commands/recovery and restricted legacy writes. T01–T13 are accepted historical work. Typed POL-02 application/runtime/compiler/solver/parity evidence and the V54 effective SQL safeguard/no-write regression are complete; POL-04 is recorded as the bounded corrective that closed the R0 gap. New P1 correctness finding: V49 adoption's nullable membership check fails to reject a missing row; SAFE-01 fixes this before new features.

The manager product is incomplete: readiness and certification are implemented only for the current bounded setup slices, broader generation/review UX remains unfinished, normal UI still exposes some internal architecture, and full print/schedule acceptance is not yet evidenced. Governed session lock/unlock controls are now present with context invalidation and effective session-or-placement protection. Copilot is tenant-scoped but remains optional and is not canonical scheduling authority. Empty-workspace provisioning, neutral onboarding, the aligned Sunday calendar contract and the bounded reviewed CSV intake are implemented; tenant-specific hour defaults, independent studio acceptance and full operational evidence remain unfinished. Passing tests do not prove private data completeness, deployed migration state or independent manager use.
...
The adopted execution route completed R1/POL-04, which closed the demonstrated POL-02 SQL safeguard gap, R2/SET-03 through SET-05, which completed the bounded studio, room, teacher, class/session and roster setup slices, R3/POL-03 typed linked policy semantics and SET-06 student relationships, R4/SET-07 readiness/certification gates, R4/UX-01 understandable generation/failure handling, R4/UX-02 editing/recovery, R4/LOCK-01 governed session locks and R4/UX-03 usable schedule export. R5/OPS-01 completed the disposable engineering rehearsal but remains blocked on external release evidence; R6/GEN-01, R7/GEN-02, R8/GEN-03, R9/GEN-04 and R10/IMPORT-01 are complete, R12/OPT-01, R13/OPT-02 and R14/CAND-01 are complete, and R15/GEN-05 is selected but blocked on EXT-STUDIO2. Later runs follow the dependency graph in [TASKS](TASKS.md). This route is planning guidance only: no deployment, merge, external participant or private-data action is implied.
...
4. GEN-01/02 finish generic policy extraction; GEN-03 completes explicit command tenancy and GEN-04 delivers empty-workspace onboarding. IMPORT-01 reduces bulk setup effort; GEN-05 proves a distinct studio.
...
| Preferences | Deterministic OPT-01 breakdown and bounded OPT-02 lexicographic optimization with independent rescore | Record honestly; feasibility + manual finish accepted | Same | OPT-01/02 measured | Repeatable quality |
...
| Locks/regeneration | Governed session lock/unlock controls and context invalidation | LOCK-01 controls/context | Generic | Supported | Repeated |
...
| Onboarding/login/auth | OAuth/magic link plus empty-workspace creation; local login rendered | Clear setup entry | Empty workspace + select | Invite/revoke verified | Repeatable |
| Imports | Reviewed CSV templates, preview, explicit-ID validation and atomic apply; manager-assigned self-service handoffs now link to the same canonical forms; independent manager acceptance remains unverified | Forms sufficient; honest effort gate | GEN-05 independent-studio acceptance | Support/limits | Stable |
| AI | Tenant-scoped context/proposal code remains optional and partial | Hide misleading paths; not required | Optional/off | Optional/off | Optional/off |
...
| Candidate comparison | Durable tenant-scoped review envelopes, exact context comparison and governed adoption revalidation | Review before adopt | No false compare claims | CAND-01 complete | Supported |
| Tenancy/roles/security | Explicit tenant-scoped commands and membership/RLS checks; lifecycle partial | GEN-03 boundary evidence; lifecycle remains | Every boundary explicit | Lifecycle/privacy | Maintained |
