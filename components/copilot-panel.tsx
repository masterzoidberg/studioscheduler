...
export function CopilotPanel({mobile=false,onClose}:{mobile?:boolean;onClose?:()=>void}){
  ...
  function proposalIsStale(){
    ...
    return (patch.baseRulebookVersion!==undefined&&patch.baseRulebookVersion!==currentRulebookVersion)||(patch.baseScheduleVersion!==undefined&&patch.baseScheduleVersion!==currentScheduleVersion);
  }

  async function apply(){
    if(!result?.proposal||!canEdit)return;
    if(proposalIsStale()){
      setNotice(`This proposal was generated against Rulebook v${result.proposal.patch.baseRulebookVersion??"?"} / Schedule v${result.proposal.patch.baseScheduleVersion??"?"}, but canonical truth is now Rulebook v${currentRulebookVersion} / Schedule v${currentScheduleVersion}. Ask Copilot again before applying.`);
      return;
    }
    setBusy(true);
