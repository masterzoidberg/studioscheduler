...
export function CopilotPanel({mobile=false,onClose}:{mobile?:boolean;onClose?:()=>void}){
  ...
  const {session,canEdit,applyRulePatch,applySchedulePatch,currentRulebookVersion,currentScheduleVersion,currentScheduleRulebookVersion,scheduleIsStale,validation}=useWorkspace();
  ...
  return <aside className={`flex h-full flex-col border-l border-slate-200 bg-white ${mobile?"rounded-t-[28px] border shadow-2xl sm:rounded-[24px]":""}`}><div className="flex items-center justify-between border-b border-slate-200 p-4"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-xl bg-slate-950 text-white"><Sparkles className="size-4"/></div><div><p className="text-sm font-semibold">AI Copilot</p><p className="text-[11px] text-slate-500">Rulebook v{currentRulebookVersion} · Schedule v{currentScheduleVersion} → Rules v{currentScheduleRulebookVersion}</p></div></div>{onClose?<button onClick={onClose} className="rounded-xl p-2 text-slate-500"><X className="size-5"/></button>:null}</div>
  <div className="flex-1 overflow-y-auto p-4"><div className="rounded-2xl bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-semibold text-slate-600"><Database className="size-4"/>Current canonical database</div><p className="mt-2 text-xs leading-5 text-slate-500">Answers use the current reviewed Rulebook and current assignments. Human review and deterministic enforcement remain separate.</p><p className="mt-2 text-[11px] text-slate-500">Coverage: {validation.coverage.implementedHardRules}/{validation.coverage.applicableHardRules} applicable HARD rules · {scheduleIsStale?"schedule needs Rulebook revalidation":"schedule linked to current Rulebook"}</p></div><div className="mt-4 flex flex-wrap gap-2">{prompts.map(p=><button key={p} disabled={busy} onClick={()=>void send(p)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-left text-xs leading-5 text-slate-600 hover:bg-slate-50">{p}</button>)}</div>{busy?<div className="mt-5 flex items-center gap-2 text-sm text-slate-500"><LoaderCircle className="size-4 animate-spin"/>Reading current studio data…</div>:null}{result?<div className="mt-5 space-y-3"><div className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.12em] text-slate-400"><Bot className="size-4"/>{result.mode==="OPENROUTER"?`OpenRouter${result.model?` · ${result.model}`:""}`:"Local database lookup"}</div><div className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{result.answer}</div></div>{result.proposal?<div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><p className="text-xs font-semibold uppercase tracking-[.12em] text-violet-700">Proposed change · not applied</p><h3 className="mt-2 font-semibold text-violet-950">{result.proposal.title||result.proposal.kind}</h3><pre className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap rounded-xl bg-white/80 p-3 text-[11px] leading-5 text-slate-600">{JSON.stringify(result.proposal.patch,null,2)}</pre><p className="mt-3 text-[11px] leading-5 text-violet-900">Apply routes through the same version checks and deterministic validation boundary used by human edits. A stale or invalid proposal is rejected.</p><div className="mt-3 flex gap-2"><button onClick={()=>setResult({...result,proposal:null})} className="min-h-10 flex-1 rounded-xl border border-violet-300 bg-white text-sm font-semibold text-violet-900">Reject</button><button disabled={busy||!canEdit||scheduleIsStale&&result.proposal.kind==="SCHEDULE_PATCH"} onClick={()=>void apply()} className="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-violet-900 text-sm font-semibold text-white disabled:opacity-40"><Check className="size-4"/>{canEdit?"Apply":"Viewer only"}</button></div></div>:null}</div>:null}{notice?<div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-900">{notice}</div>:null}</div>
  <div className="border-t border-slate-200 p-4"><textarea value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();void send();}}} rows={3} placeholder="Ask about a rule, conflict, person, or proposed change…" className="w-full resize-none rounded-xl border border-slate-300 p-3 text-sm outline-none focus:border-slate-950"/><button disabled={busy||!message.trim()||!session} onClick={()=>void send()} className="mt-2 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 text-sm font-semibold text-white disabled:opacity-40"><Send className="size-4"/>Send to Copilot</button><p className="mt-2 text-[10px] leading-4 text-slate-400">AI never silently mutates canonical data. Explicit approval and governed server mutations are required.</p></div></aside>;
}
