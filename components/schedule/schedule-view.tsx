  ...
  function detailedCard(assignment: Assignment, top: number, height: number, outside: boolean) {
    ...
    return (
      ...
      >
        ...
        {height >= 54 ? <p className="mt-1 truncate text-[11px] font-medium text-slate-700">{teacher?.name}</p> : null}
        {height >= 78 ? <p className="mt-0.5 truncate text-[10px] text-slate-500">{pretty(assignment.startTime)}–{pretty(assignment.endTime)}</p> : null}
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm leading-6 text-slate-600">Drag an unlocked class to another 15-minute slot or room. Every accepted drop is revalidated and saved as a new ScheduleVersion.</p>
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            ...
            <span className={`rounded-full border px-3 py-1.5 ${scheduleIsStale ? "border-amber-200 bg-amber-50 text-amber-800" : validation.valid ? "border-blue-200 bg-blue-50 text-blue-800" : "border-red-200 bg-red-50 text-red-700"}`}>{health}</span>
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1.5">{validation.coverage.implementedHardRules}/{validation.coverage.applicableHardRules} HARD enforced</span>
          </div>
        </div>
        <div className="text-right text-xs text-slate-500">Schedule linked to Rulebook v{currentScheduleRulebookVersion}</div>
      </section>

      {notice ? <section className={`rounded-xl border p-3 text-sm ${notice.startsWith("Move blocked") ? "border-red-200 bg-red-50 text-red-900" : "border-blue-200 bg-blue-50 text-blue-900"}`}>{notice}</section> : null}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Schedule view</p><p className="mt-1 text-sm text-slate-600">Focus on one day, compare adjacent days, or scan the full week.</p></div>
          <div className="flex flex-wrap gap-2">
            {([[1, "1 Day"], [2, "2 Days"], [3, "3 Days"], ["week", "Week"]] as const).map(([mode, label]) => (
              <button key={label} type="button" onClick={() => setViewMode(mode)} className={`min-h-10 rounded-xl px-4 text-sm font-semibold ${viewMode === mode ? "bg-slate-950 text-white" : "border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>{label}</button>
            ))}
          </div>
        </div>
        <div className="mt-4 flex items-center gap-2">
          <button type="button" onClick={() => stepDay(-1)} disabled={day === days[0]} className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 text-slate-600 disabled:opacity-30" aria-label="Previous day"><ChevronLeft className="size-4" /></button>
          <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1">
            {days.map((targetDay) => <button key={targetDay} type="button" onClick={() => scrollToDay(targetDay)} className={`min-h-10 shrink-0 rounded-xl px-4 text-sm font-semibold ${day === targetDay ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"}`}>{targetDay}</button>)}
          </div>
          <button type="button" onClick={() => stepDay(1)} disabled={day === days[days.length - 1]} className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 text-slate-600 disabled:opacity-30" aria-label="Next day"><ChevronRight className="size-4" /></button>
        </div>
        <p className="mt-2 text-xs text-slate-400">{viewMode === "week" ? "Week view is for scanning. Tap a class for details; use a detailed view to drag it." : "Each horizontal line is 15 minutes. Drag class cards to move them; drag empty canvas space horizontally to pan through days."}</p>
      </section>
      ...
      <div className="md:hidden"><div className="space-y-3">
        ...
        {dayAssignments.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">No assignments on {day} yet.</div> : null}
      </div></div>

      {viewMode !== "week" ? (
        <div ref={scheduleScrollerRef} onPointerDown={handleCanvasPointerDown} onPointerMove={handleCanvasPointerMove} onPointerUp={handleCanvasPointerEnd} onPointerCancel={handleCanvasPointerEnd} className="hidden cursor-grab snap-x snap-mandatory overflow-x-auto rounded-2xl border border-slate-200 bg-slate-100/70 pb-2 active:cursor-grabbing md:flex">
          {days.map((targetDay) => {
            const operating = windowFor(targetDay); const rows = Math.ceil((operating.end - operating.start) / 15); const gridHeight = rows * slotHeight; const targetAssignments = assignmentsFor(targetDay); const roomColumnMinimum = detailCount === 1 ? 190 : detailCount === 2 ? 145 : 112; const gridTemplateColumns = `66px repeat(${Math.max(state.rooms.length, 1)}, minmax(${roomColumnMinimum}px, 1fr))`;
            return (
              <section key={targetDay} data-schedule-day={targetDay} className="snap-start border-r border-slate-200 bg-white last:border-r-0" style={{ flex: `0 0 ${detailPanelWidth}`, minWidth: detailPanelMinWidth }}>
                <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur"><div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold text-slate-900">{targetDay}</h2><p className="text-xs text-slate-500">{targetAssignments.length} scheduled session{targetAssignments.length === 1 ? "" : "s"}</p></div>{day === targetDay ? <span className="rounded-full bg-slate-950 px-2.5 py-1 text-[10px] font-semibold text-white">FOCUS</span> : null}</div></div>
                <div className="grid border-b border-slate-200 bg-slate-50" style={{ gridTemplateColumns }}><div className="p-2 text-[10px] font-semibold text-slate-500">TIME</div>{state.rooms.map((room) => <div key={room.id} className="border-l border-slate-200 p-2 text-center"><p className="truncate text-xs font-semibold">{room.name}</p></div>)}</div>
        ...
        <div className="hidden overflow-x-auto rounded-2xl border border-slate-200 bg-white md:block"><div className="grid min-w-[960px] grid-cols-6 divide-x divide-slate-200">
      ...
      {!validation.valid ? <section className="rounded-2xl border border-red-200 bg-red-50 p-4"><h2 className="font-semibold text-red-900">Detected HARD violations</h2><div className="mt-3 space-y-2">{validation.violations.filter((item) => item.severity === "HARD").slice(0, 12).map((item, index) => <div key={`${item.constraintId}-${index}`} className="rounded-xl bg-white/70 p-3 text-sm text-red-900"><strong>{item.constraintId}</strong><span className="ml-2">{item.message}</span></div>)}</div></section> : <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950"><ShieldCheck className="mr-2 inline size-4" />No HARD conflicts are detected by the {validation.coverage.implementedHardRules} implemented HARD rules. {validation.coverage.notImplementedHardRules} applicable HARD rules still await machine enforcement.</section>}
