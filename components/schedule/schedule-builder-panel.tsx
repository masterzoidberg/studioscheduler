...
export function ScheduleBuilderPanel() {
  ...
  const eligibleTeachers = placingClass?.eligibleTeacherIds?.length
    ? state.teachers.filter((teacher) => placingClass.eligibleTeacherIds.includes(teacher.id))
    : state.teachers;
  const eligibilityMissing = Boolean(placingClass && placingClass.eligibleTeacherIds.length === 0);
  ...
  function changeDay(nextDay: Day) {
    ...
    if (nextDay !== "Saturday" && startTime < "16:15") setStartTime(nextDefault);
  }

  async function placeSession() {
    if (!placing || !placingClass || !candidate || !placementAllowed || scheduleIsStale) return;
    setSaving(true);
    setNotice("");
  ...
  async function unassign() {
    if (!pendingUnassign || pendingUnassign.locked || scheduleIsStale) return;
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2 2xl:grid-cols-3">
              {unscheduled.map((session) => {
                ...
                return <div key={session.id} className="rounded-xl border border-amber-200 bg-amber-50/60 p-3">
                  ...
                  <button type="button" disabled={!canEdit || scheduleIsStale || saving} onClick={() => beginPlace(session)} className="mt-3 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-3 text-sm font-semibold text-white disabled:opacity-40"><CalendarPlus2 className="size-4" />Place class</button>
                </div>;
              })}
        ...
        <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 sm:items-center sm:p-6">
          <div className="max-h-[94vh] w-full max-w-lg overflow-y-auto rounded-t-[28px] bg-white p-5 shadow-2xl sm:rounded-[28px] sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              ...
              {eligibilityMissing ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900"><AlertTriangle className="mr-1.5 inline size-3.5" />Teacher eligibility is not recorded for this class. Choosing a teacher here does not establish that they are qualified.</div> : null}
