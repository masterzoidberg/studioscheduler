...
import type { Assignment, RuleEnforcementMapping, RuleEnforcementProposal, RulePatch, StudioRule, StudioState } from "@/lib/domain";
...
function humanChanges(rule: StudioRule): Partial<StudioRule> {
  return { id: rule.id, category: rule.category, title: rule.title, description: rule.description, classificationRaw: rule.classificationRaw, strength: rule.strength, status: rule.status, verificationStatus: rule.verificationStatus, reviewStatus: rule.reviewStatus, review: rule.review, sourceRaw: rule.sourceRaw };
}
...
function mappingSummary(mapping: RuleEnforcementMapping, state: StudioState) {
...
function hypotheticalValidation(state: StudioState, proposal: RuleEnforcementProposal, currentAssignments: Assignment[]) {
  ...
  return validateSchedule({ ...state, enforcementVersions: [...state.enforcementVersions.map((version) => ({ ...version, status: "HISTORICAL" as const })), { ...current, id: "preview-enforcement", version: current.version + 1, reason: "Preview", changedRuleIds: [proposal.ruleId], snapshot, status: "CURRENT" as const }] }, currentAssignments);
}

export function RulebookView() {
  const { state,currentAssignments,currentRulebookVersion,currentEnforcementVersion,applyRulePatch,exportPackage,canEdit,reviewEnforcementProposal } = useWorkspace();
  ...
  function beginCreate() {
    if (!canEdit) return;
    setEditing({ id: `NEW-${String(Date.now()).slice(-3)}`, category: "General", type: null, title: "New scheduling rule", description: "Describe the scheduling policy in plain language.", strength: null, classificationRaw: "MODERATE", status: "NEEDS_REVIEW", verificationStatus: "UNVERIFIED", reviewStatus: "UNVERIFIED", review: { decision: "USER_EDIT", verified: false }, affectedEntityIds: [], parameters: {}, exceptions: [], source: { type: "USER_EDIT" }, sourceRaw: {}, enforcementStatus: "NOT_IMPLEMENTED", versionIntroduced: currentRulebookVersion + 1, updatedAt: "" });
    setOriginal(null); setReason(""); setNotice("");
  }
  async function save() {
    if (!editing) return;
    setSaving(true);
    const result = await applyRulePatch({ id: "rulebook-edit", ruleId: original?.id, operation: original ? "UPDATE" : "CREATE", changes: humanChanges(editing), reason: reason.trim() || (original ? "Rule edited in Rulebook" : "Rule created in Rulebook"), proposedBy: "USER" });
    setSaving(false);
  ...
  function ask(rule: StudioRule) { window.dispatchEvent(new CustomEvent("dwde:copilot", { detail: { message: `Explain current Rulebook rule ${rule.id} (${rule.title}), including its reviewed wording and whether Enforcement v${currentEnforcementVersion} currently maps it.` } })); }

  return <div className="space-y-5">
    <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-500">DWDE 2026–2027 Master Rulebook</p><h2 className="mt-1 text-2xl font-semibold">Rulebook v{currentRulebookVersion} · Enforcement v{currentEnforcementVersion}</h2><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-emerald-50 px-3 py-1.5 font-semibold text-emerald-700">{currentVersion?.sourceHash ? "Reviewed human authority" : "Current human authority"}</span><span className="rounded-full border border-slate-200 px-3 py-1.5">{state.rules.length} Rules</span><span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-emerald-700">{verified} Verified</span><span className="rounded-full border border-red-200 bg-red-50 px-3 py-1.5 text-red-700">{hard} active HARD</span><span className="rounded-full border border-violet-200 bg-violet-50 px-3 py-1.5 text-violet-700">{approvedMappings.size} mapped · {pendingProposals.length} pending</span></div>{currentVersion?.sourceHash ? <p className="mt-3 max-w-3xl break-all text-[11px] leading-5 text-slate-400">Reviewed source fingerprint: {currentVersion.sourceHash}</p> : null}</div><div className="flex gap-2"><button onClick={download} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 px-3 text-sm font-semibold"><Download className="size-4"/>Export</button>{canEdit ? <button onClick={beginCreate} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white"><Plus className="size-4"/>Add Rule</button> : null}</div></div><div className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-900"><ShieldCheck className="mr-1 inline size-4"/><strong>Two independent truths.</strong> Rulebook v{currentRulebookVersion} stores what Cami approved in human language. Enforcement v{currentEnforcementVersion} stores only the machine mappings explicitly approved for deterministic validation.</div></section>

    {pendingProposals.length ? <section className="rounded-2xl border border-violet-200 bg-violet-50 p-4 sm:p-5"><div className="flex items-start gap-3"><Cpu className="mt-0.5 size-5 text-violet-700"/><div><p className="text-xs font-semibold uppercase tracking-[.12em] text-violet-700">Mapping review queue</p><h3 className="mt-1 font-semibold text-violet-950">{pendingProposals.length} proposed mapping{pendingProposals.length === 1 ? "" : "s"} waiting for review</h3><p className="mt-1 text-xs leading-5 text-violet-800">Proposals do not affect the schedule until explicitly approved.</p></div></div><div className="mt-3 flex gap-2 overflow-x-auto pb-1">{pendingProposals.map((item) => <button key={item.id} onClick={() => { setProposalId(item.id); setReviewReason(""); }} className="min-h-11 shrink-0 rounded-xl bg-white px-3 text-left text-xs font-semibold text-violet-900 shadow-sm"><span className="block">{item.ruleId}</span><span className="mt-0.5 block font-normal text-violet-600">{item.proposedMapping.type}</span></button>)}</div></section> : null}
