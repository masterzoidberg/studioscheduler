...
import { Download, FileJson, LogOut, ShieldCheck, UploadCloud } from "lucide-react";
...
export function SettingsView(){
  ...
  async function accept(){
    ...
    if(accessMode==="ALPHA"){
      setNotice("Canonical import is disabled for shared alpha links. Sign in as a studio user before replacing source-of-truth data.");
      return;
    }
  ...
  function download(){
    ...
    a.href=url;a.download=`DWDE-Rulebook-v${currentRulebookVersion}.json`;a.click();
  ...
  return <div className="space-y-6">
    <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
      <div className="flex items-start gap-3">
        <div className="grid size-10 shrink-0 place-items-center rounded-xl bg-slate-100"><UploadCloud className="size-5 text-slate-500"/></div>
        <div><h2 className="font-semibold">Import canonical Rulebook</h2><p className="mt-1 text-sm leading-6 text-slate-600">Upload JSON → validate → preview → resolve modified stable IDs → accept transactionally. The database remains canonical afterward.</p></div>
      </div>
      <label className="mt-5 flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 px-4 text-center">
        <FileJson className="size-7 text-slate-400"/><span className="mt-2 text-sm font-semibold">{fileName||"Choose dwde-rulebook-vN.json"}</span><span className="mt-1 text-xs text-slate-500">Canonical format_version 1.0</span>
      ...
      {pkg&&validation?<div className="mt-5 space-y-4">
        <div className={`rounded-2xl border p-4 ${validation.valid?"border-emerald-200 bg-emerald-50":"border-red-200 bg-red-50"}`}>
          <div className="flex items-center gap-2 font-semibold"><ShieldCheck className="size-4"/>{validation.valid?"Import structure is valid":"Import cannot be accepted"}</div>
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
            <div className="rounded-xl bg-white/70 p-3"><strong className="block text-xl">{validation.summary.teachers}</strong>Teachers</div>
            <div className="rounded-xl bg-white/70 p-3"><strong className="block text-xl">{validation.summary.rooms}</strong>Rooms</div>
            <div className="rounded-xl bg-white/70 p-3"><strong className="block text-xl">{validation.summary.classes}</strong>Classes</div>
            <div className="rounded-xl bg-white/70 p-3"><strong className="block text-xl">{validation.summary.rules}</strong>Rules</div>
          </div>
          <p className="mt-3 text-xs text-slate-600">{validation.summary.hard} HARD · {validation.summary.veryStrong} VERY STRONG · {validation.summary.moderate} MODERATE · {validation.summary.light} LIGHT · {validation.summary.baseline} BASELINE</p>
        </div>
        ...
        {diff?<div className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="font-semibold">Stable-ID diff</h3>
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
            <div className="rounded-xl bg-slate-50 p-3"><strong className="block text-xl">{diff.unchanged.length}</strong>Unchanged</div>
            <div className="rounded-xl bg-amber-50 p-3 text-amber-900"><strong className="block text-xl">{diff.modified.length}</strong>Modified</div>
            <div className="rounded-xl bg-emerald-50 p-3 text-emerald-900"><strong className="block text-xl">{diff.added.length}</strong>New</div>
            <div className="rounded-xl bg-slate-50 p-3"><strong className="block text-xl">{diff.missing.length}</strong>Missing</div>
          </div>
          {diff.modified.length?<div className="mt-4 space-y-2"><p className="text-xs font-semibold text-slate-600">Resolve modified rules. No silent overwrite:</p>{diff.modified.map(id=>{const current=state.rules.find(r=>r.id===id);const incoming=pkg.rules.find(r=>r.id===id);return <div key={id} className="rounded-xl border border-slate-200 p-3"><p className="text-sm font-semibold">{current?.title||incoming?.title||id}</p><p className="mt-1 text-[11px] text-slate-400">{id}</p><div className="mt-2 flex flex-wrap gap-2"><button onClick={()=>setResolutions({...resolutions,[id]:"KEEP_CURRENT"})} className={`min-h-9 rounded-lg border px-3 text-xs font-semibold ${resolutions[id]==="KEEP_CURRENT"?"border-slate-950 bg-slate-950 text-white":"border-slate-300"}`}>Keep Current</button><button onClick={()=>setResolutions({...resolutions,[id]:"USE_IMPORT"})} className={`min-h-9 rounded-lg border px-3 text-xs font-semibold ${resolutions[id]==="USE_IMPORT"?"border-blue-700 bg-blue-700 text-white":"border-slate-300"}`}>Use Imported</button></div></div>})}</div>:null}
        </div>:null}
    ...
    <section className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5"><ShieldCheck className="size-5 text-slate-400"/><h2 className="mt-3 font-semibold">Connection boundaries</h2><p className="mt-1 text-sm leading-6 text-slate-600">Google/Supabase identity, encrypted OpenRouter credentials, AI inference, and deterministic schedule validation remain separate. OpenRouter can propose changes but cannot commit them directly.</p></div>
    </section>
