-- V4.1 canonical solver candidate intervals.
--
-- The service boundary validates the candidate against the pinned solver
-- context first. Keep this SQL check as defense in depth for the service_role
-- adoption RPC: the persisted end time must be the same effective session
-- duration that the caller supplied and the gateway validated.
...
begin
  ...
  if v_invalid>0 then
    raise exception 'CANDIDATE_INVALID_ASSIGNMENTS: % assignment row(s) failed canonical ID/day/time-grid/interval validation',v_invalid;
  end if;
