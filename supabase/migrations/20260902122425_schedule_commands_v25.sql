-- Milestone 2 / V2.5 canonical schedule command boundary.
--
-- MOVE, ASSIGN, and UNASSIGN now share one server mutation function pinned to
-- RulebookVersion + EnforcementVersion + PlanningDatasetVersion. The server owns
-- assignment end-time calculation from the effective session duration.

create or replace function public.apply_schedule_command_v25(
  p_operation text,
...
begin
  ...
  from public.rulebook_versions where studio_id=v_studio and status='CURRENT';
  select version into v_current_enforcement
  from public.rule_enforcement_versions where studio_id=v_studio and status='CURRENT';
  select version into v_current_planning
  from public.planning_dataset_versions where studio_id=v_studio and status='CURRENT';
  ...
  if v_current.rulebook_version<>v_current_rulebook then
    raise exception 'STALE_RULEBOOK_LINK: Schedule v% uses Rulebook v%, current Rulebook is v%',v_current.version,v_current.rulebook_version,v_current_rulebook;
  end if;
  if v_current.enforcement_version<>v_current_enforcement then
    raise exception 'STALE_ENFORCEMENT_LINK: Schedule v% uses Enforcement v%, current Enforcement is v%',v_current.version,v_current.enforcement_version,v_current_enforcement;
  end if;
  if v_current.planning_dataset_version is distinct from v_current_planning then
    raise exception 'STALE_PLANNING_DATASET_LINK: Schedule v% uses Planning Dataset v%, current Planning Dataset is v%',v_current.version,coalesce(v_current.planning_dataset_version,0),v_current_planning;
  end if;

  select public.validate_schedule_hard_v25(v_current.id) into v_before_validation;
  ...
  select coalesce(s.duration_minutes,c.duration_minutes), s.locked
  ...
  insert into public.schedule_versions(
    ...
    v_studio,v_new_version,v_current_rulebook,v_current_enforcement,v_current_planning,
    v_uid,v_actor,p_reason,false
  ...
  from public.assignments a
  where a.schedule_version_id=v_new_id and a.id=v_assignment_id;

  insert into public.audit_events(studio_id,actor_user_id,actor_label,action,entity_type,entity_id,detail,payload)
  values(v_studio,v_uid,v_actor,'SCHEDULE_COMMAND','ASSIGNMENT',v_assignment_id,p_reason,
    jsonb_build_object(
      ...
      'aiProposed',p_ai_proposed,
      'beforeValidation',v_before_validation,
...
revoke all on function public.apply_schedule_command_v25(text,text,text,jsonb,text,integer,integer,integer,integer,boolean) from public,anon;
grant execute on function public.apply_schedule_command_v25(text,text,text,jsonb,text,integer,integer,integer,integer,boolean) to authenticated,service_role;

create or replace function public.undo_last_schedule_change_v25(
  p_expected_schedule_version integer,
  p_expected_rulebook_version integer,
  p_expected_enforcement_version integer,
  p_expected_planning_dataset_version integer,
...
begin
  ...
  select version into v_current_planning from public.planning_dataset_versions where studio_id=v_studio and status='CURRENT';
  ...
  if v_current.rulebook_version<>v_current_rulebook or v_current.enforcement_version<>v_current_enforcement or v_current.planning_dataset_version is distinct from v_current_planning then
    raise exception 'STALE_SCHEDULING_CONTEXT: revalidate the current schedule before undo';
  end if;
  ...
  where studio_id=v_studio
    ...
    and planning_dataset_version=v_current_planning
  limit 1;

  if v_previous.id is null then
    raise exception 'NO_COMPATIBLE_UNDO: Schedule v% has no immediately previous version using Rulebook v%, Enforcement v%, and Planning Dataset v%',
      v_current.version,v_current_rulebook,v_current_enforcement,v_current_planning;
  end if;

  select public.validate_schedule_hard_v25(v_current.id) into v_current_validation;
  ...
  insert into public.audit_events(studio_id,actor_user_id,actor_label,action,entity_type,entity_id,detail,payload)
  values(v_studio,v_uid,v_actor,'SCHEDULE_UNDO','SCHEDULE_VERSION',v_new_id::text,p_reason,
    jsonb_build_object(
...
revoke all on function public.undo_last_schedule_change_v25(integer,integer,integer,integer,text) from public,anon;
grant execute on function public.undo_last_schedule_change_v25(integer,integer,integer,integer,text) to authenticated,service_role;
