-- V3.6 exact-Rulebook repair governance and server-derived roster additions.
--
-- Deterministic Rulebook repairs are compiled from one reviewed policy artifact.
-- They must fail closed if the current Rulebook changes, even if a future version
-- happens to reuse the same class names. Existing planning history/versioning is
...
declare
  ...
  v_source_hash text;
begin
  select rb.version,rb.rulebook_id,rb.rule_count,rb.source_hash
  into v_version,v_rulebook_id,v_rule_count,v_source_hash
  from public.rulebook_versions rb
  where rb.studio_id=p_studio and rb.status='CURRENT'
  for share;

  if v_version is distinct from 3
     or v_rulebook_id is distinct from 'dwde-2026-2027-master-rulebook'
     or v_rule_count is distinct from 178
     or v_source_hash is distinct from '7d03e131bd0b6a1eddafff70fd3024628215236d3d846cb156d1514329120c5b' then
    raise exception 'RULEBOOK_POLICY_CONTENT_MISMATCH: deterministic repair operations are compiled for reviewed Rulebook v3 (178 rules, source hash 7d03e131bd0b6a1eddafff70fd3024628215236d3d846cb156d1514329120c5b)';
  end if;
end
$function$;
revoke all on function private.assert_reviewed_rulebook_v3_v36(uuid) from public,anon,authenticated;

-- Strengthen the already-live v34 required-class intake without changing its
-- public signature. Every required-class insert now content-pins the Rulebook
-- before the reviewed-intake session marker is accepted.
...
-- Versioned wrapper around the tested v35 atomic repair. The shared row lock in
-- assert_reviewed_rulebook_v3_v36 holds the exact reviewed Rulebook current for
-- the duration of this transaction, then v35 performs the existing atomic edit.
create or replace function public.apply_rulebook_structure_repair_v36(
  p_class_id text,
  p_reason text,
...
begin
  ...
  return public.apply_rulebook_structure_repair_v35(
    p_class_id,p_reason,p_expected_planning_dataset_version
  );
end
$function$;
revoke all on function public.apply_rulebook_structure_repair_v36(text,text,integer) from public,anon;
grant execute on function public.apply_rulebook_structure_repair_v36(text,text,integer) to authenticated,service_role;
...
begin
  ...
  -- BAL-006 + STU-002: every current Level 4A dancer participates in the
  -- Level 4A Ballet structures that currently exist.
  if v_key in ('ballet4a','ballet4a4b') then
    v_supported:=true;
    select coalesce(array_agg(s.id order by s.id),'{}'::text[]) into v_level_ids
    from public.students s where s.studio_id=v_studio and s.level='Level 4A';
    if cardinality(v_level_ids)>0 then
      select array_agg(distinct x order by x) into v_required_ids from unnest(array_cat(v_required_ids,v_level_ids)) x;
      select array_agg(distinct x order by x) into v_rule_ids from unnest(array_cat(v_rule_ids,array['BAL-006','STU-002']::text[])) x;
      select array_agg(distinct x order by x) into v_relationships from unnest(array_cat(v_relationships,array['Level 4A Ballet participation']::text[])) x;
    end if;
  end if;
  ...
  if v_key in ('ballet4a4b','ballet4b5') then
    v_supported:=true;
    select coalesce(array_agg(s.id order by s.id),'{}'::text[]) into v_level_ids
    from public.students s where s.studio_id=v_studio and s.level='Level 4B';
    if cardinality(v_level_ids)>0 then
      select array_agg(distinct x order by x) into v_required_ids from unnest(array_cat(v_required_ids,v_level_ids)) x;
      select array_agg(distinct x order by x) into v_rule_ids from unnest(array_cat(v_rule_ids,array['BAL-007','STU-002']::text[])) x;
      select array_agg(distinct x order by x) into v_relationships from unnest(array_cat(v_relationships,array['Level 4B Ballet participation']::text[])) x;
    end if;
  end if;
  ...
  if v_key in ('ballet4b5','ballet5') then
    v_supported:=true;
    select coalesce(array_agg(s.id order by s.id),'{}'::text[]) into v_level_ids
    from public.students s where s.studio_id=v_studio and s.level='Level 5';
    if cardinality(v_level_ids)>0 then
      select array_agg(distinct x order by x) into v_required_ids from unnest(array_cat(v_required_ids,v_level_ids)) x;
      select array_agg(distinct x order by x) into v_rule_ids from unnest(array_cat(v_rule_ids,array['BAL-008','STU-002']::text[])) x;
      select array_agg(distinct x order by x) into v_relationships from unnest(array_cat(v_relationships,array['Level 5 Ballet participation']::text[])) x;
    end if;
  end if;

  -- KAR-008 + STU-002: the reviewed Rulebook establishes one daughter record
  -- and the listed enrollments. Ambiguous or missing person identity fails closed.
