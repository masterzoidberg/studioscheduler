...
--
-- This source is staged under supabase/pending before production application.
-- After Supabase records the migration, move the exact source to
-- supabase/migrations/<production-version>_planning_dataset_versions_v25.sql.
...
-- Create the initial content-addressed snapshot for every existing studio.
do $block$
declare
  v_studio uuid;
begin
  for v_studio in select id from public.studios loop
    perform private.ensure_planning_dataset_version_v25(
      ...
      null,
      'V2.5 migration',
      'Initial PlanningDatasetVersion snapshot'
    );
  end loop;
end
$block$;

-- Existing historical schedule versions predate planning-data pinning and cannot
-- honestly be made reproducible retroactively. Only the current schedule is
-- backfilled to the current snapshot. Future inserts are pinned automatically.
alter table public.schedule_versions
  add column if not exists planning_dataset_version integer;

alter table public.scenarios
  add column if not exists base_planning_dataset_version integer;
...
alter table public.schedule_versions
  drop constraint if exists schedule_versions_planning_dataset_version_fkey;
alter table public.schedule_versions
  add constraint schedule_versions_planning_dataset_version_fkey
  foreign key (studio_id, planning_dataset_version)
  references public.planning_dataset_versions(studio_id, version);

alter table public.scenarios
  drop constraint if exists scenarios_base_planning_dataset_version_fkey;
alter table public.scenarios
  add constraint scenarios_base_planning_dataset_version_fkey
  foreign key (studio_id, base_planning_dataset_version)
  references public.planning_dataset_versions(studio_id, version);

create index if not exists idx_schedule_versions_planning_dataset_fk
  on public.schedule_versions(studio_id, planning_dataset_version);
